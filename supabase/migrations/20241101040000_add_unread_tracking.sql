...
-- ============================================================================
-- UPDATE MESSAGES TABLE - Add unread tracking (idempotent)
-- ============================================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'read_by_owner') THEN
    ALTER TABLE public.messages ADD COLUMN read_by_owner BOOLEAN NOT NULL DEFAULT FALSE;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'messages' AND column_name = 'read_by_borrower') THEN
    ALTER TABLE public.messages ADD COLUMN read_by_borrower BOOLEAN NOT NULL DEFAULT FALSE;
  END IF;
END $$;
...
-- ============================================================================
-- UPDATE BORROW_REQUESTS TABLE - Add last message info (idempotent)
-- ============================================================================
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'borrow_requests' AND column_name = 'last_message_at') THEN
    ALTER TABLE public.borrow_requests ADD COLUMN last_message_at TIMESTAMPTZ;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'borrow_requests' AND column_name = 'last_message_content') THEN
    ALTER TABLE public.borrow_requests ADD COLUMN last_message_content TEXT;
  END IF;
END $$;
